...
const GENERIC_FALLBACK = { width: 1.0, height: 1.0, depth: 1.0 };
...
// ─── dimension helpers ────────────────────────────────────────────────────────
function resolveTypeDefault(type) {
 ...
 */
function resolveExplicitDims(spec) {
    ...
    if (hasWidth && hasBreadth) {
        console.warn('[PolarTwin] Both "width" and "breadth" found in component spec — ambiguous; falling back to type defaults.');
        return null;
    }
    const width = hasWidth ? spec.width : hasBreadth ? spec.breadth : null;
 ...
 */
function gridPack(childDims, gap = 0.6) {
    ...
    const rowDepths = Array(rows).fill(0);
...
function classifyTypeTier(type) {
    const t = (type || '').toLowerCase();
 ...
 */
function buildNodeMap(node, parentWorldOrigin = [0, 0, 0], parentName = null, parentAncestors = new Set()) {
    const map = new Map();
...
function wallAttachPoint(from, to) {
 ...
 */
function isInsideAny(wx, wz, obstacles) {
 ...
 */
function aStarRoute(startW, endW, obstacles, usedCells, bounds) {
    ...
    const h = (gx, gz) => Math.abs(gx - egx) + Math.abs(gz - egz);
    const open = new Map();
...
function simplifyPath(pts) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
        ...
        if (sameDirX <= 0 || sameDirZ <= 0) {
            ...
            const isDifferentDir = Math.sign(prevDir[0]) !== Math.sign(nextDir[0]) ||
...
function findLCA(src, tgt, allNodes) {
    const srcAncestors = new Set();
    let curr = src.name;
    while (curr) {
 ...
 */
function routeConnection(srcInfo, tgtInfo, allNodes, srcName, tgtName, usedCells) {
    ...
    for (const [name, info] of allNodes) {
        if (srcAnc.has(name) || tgtAnc.has(name))
            continue;
        obstacles.push({
    ...
    const isSegmentClear = (sx, sz, ex, ez) => {
    ...
    if (!rawPath) {
        rawPath = aStarRoute(startW, endW, obstacles, usedCells, bounds);
    }
    ...
    if (!rawPath) {
        rawPath = aStarRoute(startW, endW, leafObstacles, usedCells, bounds);
    }
    ...
    if (!rawPath) {
        ...
        rawPath = aStarRoute(startW, endW, leafObstacles, // Keep avoiding leaf components!
        usedCells, expandedBounds);
    }
    if (!rawPath) {
 ...
 */
export function buildLayout(node, spec, rawConnections = []) {
    const name = node.name ?? '(unnamed)';
    const type = node.type ?? '';
    const tags = node.tags ?? [];
    const level = typeof node.level === 'number' ? node.level : undefined;
    const rawSpec = (spec?.components?.[name]?.spec) ?? {};
    const rawChildren = node.children ?? [];
    // Build children first so their dims are known.
    const children = rawChildren.map((c) => buildLayout(c, spec, rawConnections));
 ...
 */
export function buildSceneLayout(topology, connectionsData, spec) {
    ...
    const floorContainers = new Map();
    const collectFloors = (node, parentName) => {
        if (node.type === 'floor' && parentName) {
            if (!floorContainers.has(parentName))
                floorContainers.set(parentName, []);
    ...
    const connections = rawConnections
        ...
        if (!src || !tgt) {
            console.warn(`[PolarTwin] Connection ${c.source}→${c.target}: ` +
        ...
        const path = routeConnection(src, tgt, nodeMap, c.source, c.target, usedCells);
        if (path.length === 0) {
            return null;
        }
        ...
        const elevatedPath = path.map(([x, y, z]) => [x, y + elevation, z]);
        return {
